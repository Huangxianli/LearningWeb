...
export abstract class ActiveEffectClass {
  ...
  abstract _running: number; // 用于控制防止进入死循环（一个副作用函数里面又读又写）
...
// 执行副作用函数前的操作，主要是为了后续执行副作用函数的过程中收集依赖不重复收集 key（一个 key 在副作用行数中读多次，只应该收集一次） 和删除不在依赖的 key（一个 key 在副作用函数中，有的分支中读了，在有的分支中没有读）
...
// 如果一个被包裹的副作用函数，第二次执行时，最后几个 key 不再被读，那就要进行清除掉最后的依赖
function afterRunEffect(activeEffect: ActiveEffect) {
  ...
  for (let i = depLength; i < lastDepLength; i++) {
    if (
      activeEffect.depsKeyDepsMap[i].get(activeEffect) === activeEffect._trackId
    ) {
...
export function trackEffect(
  activeEffect: ActiveEffectClass,
  keyDepsMap: KeyDepsMap
) {
  if (activeEffect._trackId !== keyDepsMap.get(activeEffect)) {
    // 一个副作用函数中读取了一个 key 多次时，当前 key 只收集当前副作用函数一次
    ...
    const oldKeysDepMap =
      activeEffect.depsKeyDepsMap[activeEffect._depsKeyDepsMapLength];
