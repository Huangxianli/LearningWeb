...
/**
 * 判断类型兼容的方式（这里主要使用条件类型来判断兼容性；还有一种是通过赋值来进行兼容性检查，没报错就说明兼容）
 * 子类型应该是更加明确的类型
 */
...
function test1() {
  ...
  type Type2 = string | number extends Type1 ? 1 : 2; // Type2 2 (string | number) 类型不是 string 类型的子类型
}
...
function test2() {
  ...
  // 注意 Object 类型是除了 null 和 undefined 之外的所有类型 object 是除了原始类型外的所有引用类型（不确定是否有原型），{} 是和 Object 一样的，但是不能添加属性等
...
function test3_1() {
  ...
  type Result3 = 1 | '' | '12' extends string ? 1 : 2; // Result3 2 只要联合类型中的任何一个子类型不满足条件，extends 就会走 false 逻辑
  type Result4 = 1 | '' | '12' extends string | number ? 1 : 2; // Result4 1 左边联合类型的所有子类型都满足右边联合类型的任何一个类型，extends 就会走 true 逻辑；否之，走 false 逻辑
}
...
function test3_3() {
  ...
  // unknown 类型和 any 类型是不同的，它只能赋值给 unknown 类型和 any 类型，可以接收任何类型的赋值，unknown extends 任何类型 都应该为 true
...
function test4() {
  ...
  // null undefined void（如果设置了 strictNullChecks: true null undefined  不是任何类型的子类型）
...
function test5() {
  ...
  // 数组和元组
  type Result3 = [number, number] extends number[] ? 1 : 2; // 1
  type Result4 = [number, string] extends number[] ? 1 : 2; // 2 string extends number 为 false
  type Result5 = [any] extends number[] ? 1 : 2; // 1  和单独的 any 不同， any extends number ? 1: 2 是 1 | 2
  type Result6 = number[] extends (string | number)[] ? 1 : 2; // 1
  type Result7 = unknown[] extends number[] ? 1 : 2; // 2
  type Result8 = null[] extends number[] ? 1 : 2; // 2
