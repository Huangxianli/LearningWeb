...
/**
 * 字面量类型
 * 字面量类型比原始类型更加得到精确，主要有数字字面量、BigInt字面量、字符串字面量、布尔字面量、符号字面量（unique symbol）、枚举字面量、对象字面量
 * 字面量类型要求的是值级别的一致
 *
...
function test1_1(): void {
  ...
  // const c: typeof symbol1 = Symbol(); // 会报错，typeof Symbol() 会被推断成 unqiue symbol 类型，而不是 symbol 类型
}
 ...
 * 枚举值是延迟求值
 * 枚举值是双向映射的，可以从枚举成员映射到枚举值，也可以从枚举值映射到枚举成员（注意只有枚举值是数字时才能进行双向映射，即使是字符串也不行）
 *
