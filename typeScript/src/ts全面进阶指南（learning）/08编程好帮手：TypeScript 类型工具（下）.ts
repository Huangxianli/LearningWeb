 ...
 *
 * 注意，typeof 如果是在 ts 中的话，更看重定义时的类型（例如 type A = typeof c），如果是在 js 中的话，更看重执行时的类型（例如 let a = typeof c）
 */
function test1() {
  console.log('---test1---------------------------------------------');
  ...
  const obj3 = { 1: 2 };
  const obj4: typeof obj3 = { 1: 3 };

  const obj5 = { 1: { 2: {}, 3: 1 } };
  type Obj5 = typeof obj5; // type Obj5 = { 1: { 2: {}, 3: number } } // 注意这里的 const 变量，非基础类型和基础类型转化成类型时的不同表现

  const arr1 = [1, 1];
  type Arr2 = typeof arr1; // number[] 这里的每一项都是具体的类型而不是具体的值

  const symbol1 = Symbol();
  type Symbol1 = typeof symbol1; // unique symbol 推导出来带一个 unique
  const symbol2: Symbol1 = symbol1; // 这里只能等于 symbol1，是由于 Symbol 是 unique symbol，每个 unique symbol 变量都具有唯一性，只能等于自己
 ...
 *
 * interface 使用 extends 继承，不兼容直接报错
 * type 使用 & 继承，不兼容的部分直接推断成 never
 */
