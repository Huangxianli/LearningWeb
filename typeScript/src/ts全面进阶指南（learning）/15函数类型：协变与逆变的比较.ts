...
function test2(): void {
  ...
  type CheckReturnType = AsFuncReturnType<Animal> extends AsFuncReturnType<Dog>
    ? 1
    : 2; // 2
 ...
 */
function test4(): void {
  // property 的方式，如果配置了 method-signature-style 为 ture 的话，在接口中，只有这种方式定义的函数才会对函数参数进行逆变的检查
  interface T1 {
  ...
  const fn1: (arg: string[]) => void = (arg: (string | number)[]) => void 0;
  const fn2: (arg: string) => void = (arg: string | number) => void 0;
