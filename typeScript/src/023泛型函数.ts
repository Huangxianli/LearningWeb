...
firstElement1([1, '2'] as const); // T 被推断成 1 | '2'
firstElement1<string>(['1', '1']); // 调用的时候，可以指定 T 代指的类型
// firstElement<string>(["1", 1]); // 会报错

// 可以定义多个泛型
function myMap<I, O>(arr: I[], fun: (arg: I) => O): O[] {
  return arr.map(fun);
}
...
function compareLength<
  I extends { length: number },
  O extends { length: number }
>(a: I, b: O): boolean {
...
function minLength<T extends { length: number }>(obj: T, length: number): T {
  if (obj.length > length) {
    ...
    // return data; // 这里会报错，虽然返回的内容含有 length 属性，他只是满足了最小的条件，可能还有很多其他的条件没有满足
    return obj;
  }
}
