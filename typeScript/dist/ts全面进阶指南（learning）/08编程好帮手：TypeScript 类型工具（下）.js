...
function test1() {
    ...
    let str1 = 'link';
    let obj1 = { 1: 2 };
    const obj2 = { 1: 12 }; // const obj1: {1: number} 注意这里的 1 对应的类型是 number 而不是 2，无论 obj1 是变量还是常量
    const obj3 = { 1: 2 };
    const obj4 = { 1: 3 };
    const obj5 = { 1: { 2: {}, 3: 1 } };
    const arr1 = [1, 1];
    const symbol1 = Symbol();
    const symbol2 = symbol1; // 这里只能等于 symbol1，是由于 Symbol 是 unique symbol，每个 unique symbol 变量都具有唯一性，只能等于自己
    const func = (input) => {
        return input.length > 10;
    };
    let func2; // 这样 func2 的类型是取的 func 的类型
}
/**
 * 类型守卫
 */
...
function test2_1() {
    ...
    function foo(input) {
        if (typeof input === 'string') {
            ...
            const temp = input; // never 是只能接收 never 的赋值的，这里不报错，说明已经推导出了 input 在这个是 never 类型
        }
    }
}
function test2_2() {
